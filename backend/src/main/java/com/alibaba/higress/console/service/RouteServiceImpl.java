...
public class RouteServiceImpl implements RouteService {
    ...
    @Override
    public Route patch(Route route, PatchMode mode) {
        JsonPatch patch = kubernetesModelConverter.buildPatch(route, mode);
        if (patch.isEmpty()) {
            return query(route.getName());
        }
        V1Ingress ingress;
        try {
            ingress = kubernetesClientService.patchIngress(route.getName(), patch);
        } catch (ApiException e) {
            throw new BusinessException(
                "Error occurs when patching the ingress associated with route named: " + route.getName(), e);
        }
